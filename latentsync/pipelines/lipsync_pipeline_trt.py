...
class LipsyncPipelineTRT(DiffusionPipeline):
    ...
    def denoising_unet_trt_infer(self, denoising_unet_input, t, audio_embeds):
        ...
        # 分配显存方式
        # 将数据从 GPU 复制到 TensorRT 输入缓冲区
        # cuda.memcpy_dtod(self.inputs[0], denoising_unet_input.data_ptr(), denoising_unet_input.numel() * denoising_unet_input.element_size())
        # cuda.memcpy_dtod(self.inputs[1], t.data_ptr(), t.numel() * t.element_size())
        # cuda.memcpy_dtod(self.inputs[2], audio_embeds.data_ptr(), audio_embeds.numel() * audio_embeds.element_size())

        # Use data_ptr
        noise_pred_cuda = torch.empty((2, 4, 16, 32, 32), dtype=torch.float16, device='cuda')
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # 计算需要填充的长度
        # It should be a multiple of 16 to align with TensorRT Inference
        padding_length = (16 - (len(whisper_chunks) % 16)) % 16  # 确保计算正确，即使 current_len 是 16 的倍数
        if padding_length > 0:
            last_chunk = whisper_chunks[-1]
            whisper_chunks.extend([last_chunk] * padding_length)
