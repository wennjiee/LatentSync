...
def load_trt_engine(engine_path):
    ...
    with open(engine_path, "rb") as f, trt.Runtime(logger) as runtime:
        return runtime.deserialize_cuda_engine(f.read())
vae_encoder_engine = load_trt_engine("trt_engines/vae_encoder.trt")
context = vae_encoder_engine.create_execution_context()

# 输入输出 tensor
input_tensor = torch.randn(16, 3, 256, 256).cuda().half()
output_tensor = torch.empty(16, 4, 32, 32).cuda().half()
a = output_tensor
bindings = [int(input_tensor.data_ptr()), int(output_tensor.data_ptr())]
context.execute_v2(bindings)
print(a==output_tensor)
print("Success! VAE encoder TRT ran. Output shape:", output_tensor.shape)
